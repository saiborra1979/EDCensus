...
from funs_support import cvec, ymdh2date, r2_fun, ymd2date, sens_spec_df, add_CI
...
act_y = act_y.query('date >= @date_mi')
...
gg_act_y = (ggplot(act_y, aes(x='date', y='y')) + theme_bw() +
                geom_line(size=0.25,color='blue',alpha=0.75) + labs(y='Share') +
...
tmp = add_CI(res_sp_agg.query('pred==1').rename(columns={'den':'n'}))
tit = 'Precision/Recall for predicting Δ>0 in escalation\n95% CI (beta method)'
gg_sp_agg = (ggplot(tmp, aes(x='lead', y='value', color='metric')) +
...
tmp = add_CI(res_sp_full.query('pred > 0').rename(columns={'den':'n'}))
gg_sp_full = (ggplot(tmp, aes(x='lead', y='value', color='pred.astype(str)')) +
             ...
             facet_wrap('~metric', labeller=labeller(metric=di_pr)) +
              scale_color_discrete(name='Δ esclation'))
...
best_day = df_r2[(df_r2.model == models[0]) & (df_r2.month >= 3)].groupby(cn_date).r2.min().sort_values(
    ascending=False).head(1).reset_index().drop(columns=['r2'])
...
df_range = df_sens[(df_sens.model == models[0]) & (df_sens.dates.isin(drange))][
    ['lead', 'pred', 'dates', 'se']].reset_index(None, True)
...
tit = 'Example of real-time trajectory for %i hours ahead\nDay %s\nBlock dots are actual values' % (
mx_leads, best_day.strftime('%b %d, %Y'))
